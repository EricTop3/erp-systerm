    ...
    <div class="row">
      ...
      <div class="col-lg-10">
        ...
        <ol class="breadcrumb breadcrumbs">
          ...
          <li class="active"><a v-link="{ path: '/admin/dataCenter/instock/allocateList'}"> 调拨单统计</a></li>
        ...
        <div class="page-header">
          <form class="form-inline">
            <div class="form-group">
              <label>调入仓库</label>
              <select class="form-control" v-model="searchData.in_warehouse_id">
            ...
            <a v-link="{ path: '/admin/dataCenter/instock/allocateListDetail'}" class="btn btn-primary spanblocks fr mr10">明细</a>
          </form>
        </div>
        ...
        <table class="table table-striped table-border table-hover mt20" :data="onedata">
          <tbody>
          <tr>
            <td><span class="pr50">调拨数量汇总：{{onedata.amount}}</span> <span class="pr50">调拨金额汇总：{{onedata.price}}</span></td>
          </tr>
          </tbody>
        </table>
...
//      获取数据列表
      getListData: function(page){
        var self = this
        var url = requestSystemUrl + '/backend-system/data-center/stock/allocate-list'
        var data = {
...
//      获取单条数据
      getOneData: function(page){
        var self = this
        var url = requestSystemUrl + '/backend-system/data-center/stock/allocate-list/total'
        var data = {
          start_time: self.searchData.start_time,
          end_time: self.searchData.end_time,
          item_code: self.searchData.item_code,
...
//      导出 /backend-system/data-center/purchase/order
      exports: function () {
        var url = requestSystemUrl + '/backend-system/' + token + '/export' + '/data-center/stock/allocate-list'
        var data =
          'item_name=' + this.searchData.item_name + '&' +
          'item_code=' + this.searchData.item_code + '&' +
          'in_warehouse_id=' + this.searchData.in_warehouse_id + '&' +
          'out_warehouse_id=' + this.searchData.out_warehouse_id + '&' +
