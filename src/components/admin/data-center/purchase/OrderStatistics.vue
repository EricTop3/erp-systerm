      ...
      <div class="col-lg-10">
        ...
        <div class="page-header">
          <form class="form-inline">
            ...
            <a v-link="{ path: '/admin/dataCenter/purchase/orderStatistics/detail'}" class="btn btn-primary spanblocks fr mr10">明细</a>
          </form>
        </div>
        ...
        <table class="table table-striped table-border table-hover mt20" :data="onedata">
          <tbody>
          <tr>
            <td><span class="pr50">采购数量汇总：{{onedata.demand_amount}}</span> <span class="pr50">采购金额汇总：{{onedata.demand_price}}</span></td>
          </tr>
          </tbody>
        </table>
...
//      获取数据列表
      getListData: function(page){
        var self = this
        var url = requestSystemUrl + '/backend-system/data-center/purchase/order'
        var data = {
...
//      获取单条数据
      getOneData: function(page){
        var self = this
        var url = requestSystemUrl + '/backend-system/data-center/purchase/order/total'
        var data = {
          start_time: self.searchData.start_time,
          end_time: self.searchData.end_time,
          item_code: self.searchData.item_code,
...
//      导出 /backend-system/data-center/purchase/order
      exports: function () {
        var url = requestSystemUrl + '/backend-system/' + token + '/export' + '/data-center/purchase/order'
        var data =
